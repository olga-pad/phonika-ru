...
@media (max-width:700px){
  ...
  /* Four-zone shell: header/tabs, progress, flexible activity stage, controls. */
  #childView .reading{
    flex:1 1 auto;min-height:0;height:auto;max-width:100%;overflow:hidden;
  ...
  #childView .actions,#childView #lettersView .actions{
    flex:0 0 auto;width:min(100%,440px);grid-template-columns:1fr;
    gap:clamp(5px,1dvh,9px);margin:0 auto;padding-bottom:0;
  }
  #childView .actions button{
    min-height:clamp(46px,7dvh,52px);padding:clamp(6px,1dvh,9px) 14px;
    border-radius:14px;font-size:clamp(14px,3.8vw,16px);line-height:1.1;gap:8px;
  }
  #childView .actions button svg{width:clamp(22px,6vw,25px);height:clamp(22px,6vw,25px)}

  /* Progress reserves its own edge gutters so dinosaur and finish flag stay inside viewport. */
  #childView .dino-track{
    box-sizing:border-box;width:100%;height:clamp(54px,9dvh,68px);margin:0 auto!important;
    padding:0 clamp(42px,12vw,50px)!important;transform:none!important;overflow:hidden!important;
  }
  ...
  /* Learning content scales on both axes: width gives natural growth, short screens cap it earlier. */
  #childView .word,#childView .letter-card{font-size:clamp(76px,min(29vw,18dvh),138px);max-width:100%;line-height:1}
  #childView .picture,#childView .letter-picture{
    height:clamp(62px,12dvh,94px);min-height:clamp(62px,12dvh,94px);margin:clamp(1px,.7dvh,5px) 0 0;
    font-size:clamp(54px,min(19vw,11dvh),76px);
  }
  #childView .reward{min-height:clamp(8px,1.8dvh,16px)}

  /* Arrows remain inside the flexible activity zone and never determine page geometry. */
  #childView .lesson-nav{inset:0;overflow:hidden}
  #childView .nav-arrow{
    width:clamp(44px,12vw,48px);height:clamp(44px,12vw,48px);min-height:44px;top:50%;
  }
  ...
  /* Same shell primitives are safe for game screens without migrating game mechanics here. */
...
document.write('<script src="./games/games.js?v=2"><\/script><script src="./games/find.js?v=2"><\/script><script src="./games/build-word.js?v=2"><\/script><script src="./games/missing-word.js?v=2"><\/script><script src="./games/catch.js?v=1"><\/script>');

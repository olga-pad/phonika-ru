...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  const offerContinue=(button)=>{button.disabled=false;button.dataset.continueNext='1';button.innerHTML='<span>Продолжить</span>'+icon('right');};letterHelp.onclick=()=>{const x=currentLetter();if(!x)return;offerContinue(letterMastery);speak(x[1]);};if(help)help.onclick=()=>{if(!current)return;usedHint=true;offerContinue(wordMastery);speak(current[0],.68);};
  letterPictureBtn.onclick=()=>{const x=currentLetter();if(!x)return;letterPicture.hidden=!letterPicture.hidden;setActionLabels();offerContinue(letterMastery);};wordPictureBtn.onclick=()=>{if(!current)return;usedHint=true;wordPicture.hidden=!wordPicture.hidden;setActionLabels();offerContinue(wordMastery);};
